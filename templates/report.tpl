      ...
      <div class="margin_bottom_large">
        <div id="rb_expand_contract">
          {if $expand_by_default == "yes"}
            {$L.phrase_contract_all}
          {else}
            {$L.phrase_expand_all}
          {/if}
        </div>
        <div id="rb_expand_label" class="hidden">{$L.phrase_expand_all}</div>
        <div id="rb_contract_label" class="hidden">{$L.phrase_contract_all}</div>
        {$L.text_reports_intro}
      </div>
    {/if}
        ...
        <div class="rb_form_section_heading">
          <div class="rb_num_reports">
            ...
            {elseif $form_views.$key|@count == 1}
              1 {$L.word_report}
            {else}
              {$form_views.$key|@count} {$L.word_reports}
            {/if}
          </div>
          ...
          {foreach from=$form_views.$key item=view_info}
            <div class="rb_view_row">
              ...
              <div class="rb_export_options">
                <div class="rb_num_submissions">{$L.word_results_c} {$view_info.num_submissions_in_view}</div>
                {foreach from=$export_options item=export_option}
                  {assign var=group_id value=$export_option.group_id}
                  <div class="rb_export_option">
                    <a href="{$g_root_url}/modules/report_builder/report.php?export_group_id={$group_id}&export_type_id={$export_option.export_type_id}&form_id={$form_id}&view_id={$view_info.view_id}"
